...
const load=async()=>{await loadProfile(); if(!isValidUuid(id.value)||!isValidUuid(company.value?.id)){errorMessage.value='Invalid request or supplier account.';loading.value=false;return} const r=await supabase.from('wanted_requests').select('*, companies(name), vehicles(*, vehicle_makes(name), vehicle_models(name))').eq('id',id.value).maybeSingle(); request.value=r.data; const p=await supabase.from('parts').select('id,title,price').eq('seller_company_id',company.value.id).in('status',['active','draft']).order('title');parts.value=p.data||[]; const e=await supabase.from('wanted_offers').select('*').eq('request_id',id.value).eq('supplier_company_id',company.value.id).in('status',['pending','accepted']).maybeSingle();existing.value=e.data; if(e.data)Object.assign(form,{part_id:e.data.part_id||'',price:String(e.data.price||''),currency:e.data.currency,freight_price:e.data.freight_price==null?'':String(e.data.freight_price),eta_text:e.data.eta_text||'',message:e.data.message||''});loading.value=false}
onMounted(load)

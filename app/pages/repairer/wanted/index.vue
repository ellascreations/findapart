...
const load=async()=>{
  ...
  let query=supabase.from('wanted_requests').select('*, vehicles(*, vehicle_makes(name), vehicle_models(name)), wanted_offers(id,status)').eq('repairer_company_id',company.value.id).order('created_at',{ascending:false})

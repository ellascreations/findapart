...
const load=async()=>{
  ...
  const {data,error}=await supabase.from('wanted_requests').select('*, vehicles(*, vehicle_makes(name), vehicle_models(name))').eq('id',id.value).maybeSingle()
...
<section class="section"><div class="container">
  ...
  <template v-else-if="request">
    ...
    <div class="card" style="padding:22px;margin-top:20px"><p><strong>Vehicle:</strong> {{vehicleLabel(request.vehicles)}}</p><p><strong>Preferred colour:</strong> {{request.preferred_colour||'Any'}}</p><p><strong>Condition:</strong> {{request.condition_notes||'Any'}}</p><p><strong>Required by:</strong> {{request.required_by||'Not specified'}}</p><p><strong>Notes:</strong> {{request.notes||'—'}}</p></div>
